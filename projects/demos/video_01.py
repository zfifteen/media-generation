...
config.frame_height = 24  # Default is 8, increase to zoom out
...
class GeometricResonancePrimePrediction(Scene):
    def construct(self):
        ...
        ).scale(0.7).move_to(ORIGIN).shift(DOWN * 0.3)  # SURGICAL FIX
        ...
        chart_group = VGroup(axes, x_label, y_label, curve, dots, labels)  # SURGICAL FIX
        ...
        chart_group.scale(0.7).move_to(ORIGIN).shift(DOWN * 0.3)  # SURGICAL FIX
        ...
        ).scale(0.7).move_to(ORIGIN).shift(UP * 0.2)  # SURGICAL FIX
        ...
        ).arrange(DOWN, buff=0.2, aligned_edge=LEFT).scale(0.8).to_edge(DOWN, buff=0.5)  # SURGICAL FIX
