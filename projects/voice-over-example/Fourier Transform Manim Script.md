# Fourier Transform -- 5-Minute Narrated Manim Scene

Below is a complete, production-ready Manim script with ElevenLabs voiceover integration. The narration totals ~782 words, targeting roughly **5.2 minutes** at a natural speaking pace of 150 wpm. Each voiceover block uses `tracker.duration` and `tracker.get_remaining()` for tight audio-visual sync.[^1][^2]

## Rendering

```bash
# Development (free gTTS, no ElevenLabs credits burned)
manim fourier_voiceover.py FourierExplainer

# Production (ElevenLabs)
MANIM_VOICE_PROD=1 manim fourier_voiceover.py FourierExplainer
```
...
| Section | Topic | Est. Duration | Key Animations |
|---|---|---|---|
| 1 | Title card | ~19 s | Write title, fade subtitle |
| 2 | Prism analogy | ~37 s | Prism with rainbow arrows |
| 3 | Sine wave properties | ~30 s | Axes + sine curve + labels |
| 4 | Frequency sweep | ~22 s | ValueTracker animates frequency 0.5 to 4 Hz |
| 5 | Amplitude sweep | ~20 s | ValueTracker animates amplitude 0.2 to 1.8 |
| 6 | Wave superposition | ~31 s | Three waves merge into combined signal |
| 7 | Frequency spectrum | ~26 s | Bar chart of frequency components |
| 8 | Fourier integral | ~30 s | LaTeX formula with annotations |
| 9 | Circle wrapping | ~38 s | Signal wound around circle, center of mass shifts |
| 10 | Applications | ~35 s | Grid of application cards |
| 11 | Outro | ~24 s | Closing text fade |
| **Total** | | **~5 min 12 s** | |

## Key Implementation Notes

- **Dev/Prod toggle**: The `MANIM_VOICE_PROD` environment variable switches between free gTTS (for iteration) and ElevenLabs (for final render). This avoids burning ElevenLabs credits during development.[^1]
- **Audio caching**: Unchanged voiceover text will not re-generate audio on subsequent renders, saving credits.[^1]
- **Sync strategy**: Every `with self.voiceover(...)` block uses either `tracker.duration` to stretch animations or `tracker.get_remaining()` to pad the end with a wait, ensuring audio and visuals stay aligned.[^1]
- **Safe positioning**: All content stays within the +/-7 horizontal and +/-4 vertical safe bounds defined in the template.[^2]
- **Voice ID**: Uses `rBgRd5IfS6iqrGfuhlKR` as specified in the Space configuration.
- **Config values**: Locked at `frame_height=10`, `frame_width=10*16/9`, `pixel_height=1440`, `pixel_width=2560`.[^2]

## Prerequisites

```bash
pip install --upgrade "manim-voiceover-plus[elevenlabs]"
brew install sox          # macOS (or apt install sox on Linux)
export ELEVEN_API_KEY="your_key_here"
```

If you are on Python 3.13+, swap the import lines to use `manim_voiceover_plus` instead of `manim_voiceover`.[^1]

---

## References

1. [manim_voiceover.md](https://ppl-ai-file-upload.s3.amazonaws.com/web/direct-files/collection_964b162f-c1cc-4e60-aadb-25d43cdf884e/caba0c14-bffd-4833-933b-fda9398c364f/manim_voiceover.md?AWSAccessKeyId=ASIA2F3EMEYETVIMLBZL&Signature=sPa3Ms8xY11BxrL8sKpZqLMigVo%3D&x-amz-security-token=IQoJb3JpZ2luX2VjENP%2F%2F%2F%2F%2F%2F%2F%2F%2F%2FwEaCXVzLWVhc3QtMSJHMEUCIQC1JfCgN4PTHOHyddD293Svxz1t2oinj8A64JLWBjprCwIgXuUDuXdmsFIAq2M%2FGQADy%2FFLa%2BenaY7u4z0ldzjYLUwq%2FAQInP%2F%2F%2F%2F%2F%2F%2F%2F%2F%2FARABGgw2OTk3NTMzMDk3MDUiDKvQbBqEJTX4OWZ6dCrQBBsqzdyUbtE3aGaSADLd48CDo0v%2B5f3nOMrCWk2y%2B260M917DUTt5r%2FerA5q%2FBCg%2BnvUwVt0We11QbSmitjYezWfZ4boODuhPDkfO6FF2bYmJdGKvy7i7ZWVQf0GFVtcS33qUSfG%2FnpJbwnG1hQRNCnTWxZ7wpIbrpfauo6d5pSsTUpLA4G%2F0TOHj7gUvPKWWJfqiVv5boipvJU75b%2BKX3YHyLtZaM7Yag9X95lo84GBpaJ%2Fi3yFBnTZBRUy%2FVw5PP3ymGutJcos7%2BRvjbIjlXisA%2BXAm2w3SCpYZ%2FG5CUnO3b%2FsKSdhULrEP%2BthwX%2FFc7QA7XXpITdaiVXhEZ3iJBFVrSWRYIiDzbodoLRBavA8RwQnE0n71CEj3r7vdTYd8CdTREQOx%2FLp64XbyTJr8FbyJ%2FwESsUEI0IOeIt6U1vDrUpRGDJbx12E2HhZJyWTsmgKWea6IMpgnnyC8QazRj7MghlqWpoWoAuW6qPLOgQQNgdnHjxDt%2FfwOUbgDkaJZ8tqjK%2FZertdEbSeeUHxNPzlwHPybC%2BNO1lZXlNvwuguitZSlwJQJvZIUSAMNC9D6WQpT6dt%2BPpSKlBOlIkPkBClFYYVTqbWF%2FZn1Kffgse1749HJzzdEJHLycj6wIh41H%2FOjDP3jq41m6RnVdmWLxAOMPd4VkGGX3oN%2FG%2Bw01gqD88eL8inuR9cC8QhWOFtCDu2We3EnhXcotMIPO%2BiVIAdP0TcIvgq1PUn1DuVrRL03XTseXbb%2BrkbUNYEwAKLy%2B%2F6AhxSQc1i1f6OYcIq1kcw%2BbuqzAY6mAHDItjAhNyabBFeJKE1IaHbnpcZrHHXdbvT%2BZ7D1IeTWvhPAfV1EAFRzmO0A%2FVMmR3EeAl%2B%2FEKzrPM%2BqfC1x7HzGYJyYIbb4PSnfl%2F3%2FsxIyUA4FyPx0xtYhYwEDXmZIh%2FQsjPD8kXhmzwHpouGMzRAbmkauWJ0uC5InIINRJvWkqDAFjvgj%2BQ%2BADXf77MPUBJFV7UCi5cFZA%3D%3D&Expires=1770697262) - # manim-voiceover Integration Guide

## Overview

`manim-voiceover` is the official Manim Community ...

2. [manim_template.py.txt](https://ppl-ai-file-upload.s3.amazonaws.com/web/direct-files/collection